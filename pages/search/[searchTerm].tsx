      ...
      {isAccounts ? (
          ...
          {searchedAccounts.length > 0 ? (
            searchedAccounts.map((user: IUser, idx: number) => (
              <Link href={`/profile/${user._id}`} key={idx}>
                <div className="flex items-start gap-3 p-2 cursor-pointer font-semibold rounded border-b border-gray-200">
                  <div>
                    <Image
                      src={user.image}
                      alt="user profile"
            ...
            <NoResults text={`No accounts were found for ${searchTerm}`} />
          )}
        </div>
      ) : (
        <div className="md:mt-16 flex flex-wrap gap-6 md:justify-start">
          {videos.length ? (
            ...
            <NoResults text={`No videos were found for ${searchTerm}`} />
          )}
        </div>
      )}
    </div>
